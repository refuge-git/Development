    ...
    try:
        ...
        if atendimentos_por_local_dorme:
            ...
            for entry in atendimentos_por_local_dorme:
                local_raw = entry.get('localDorme', 'Desconhecido')
                numero = entry.get('numeroAtendimentosLocalDorme', 0)
                mapa_local_dorme[local_raw] = numero

                locais = [
                    'dormem em casa',
                    'dormem na rua',
                    'dormem em centros de acolhida',
                    'dormem em pensão',
                    'estão de passagem pela cidade',
                ]

            for local in locais:
                numero = mapa_local_dorme.get(local, 0)
                linhas.append(f"Onde o usuário dorme: [Nº de usuários que{local}]\t{numero}")
